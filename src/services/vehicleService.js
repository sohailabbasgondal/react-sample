...
function supplierStoreUrl(id) {
  if (id)
    return `/companies/${auth.getCurrentUser().company_id}/vehicles/${id}`;
  else return `/companies/${auth.getCurrentUser().company_id}/vehicles`;
}
...
export function updateSingleStatus(supplier) {
  return http.get(
    `/companies/${auth.getCurrentUser().company_id}/vehicles/${
      supplier.id
    }/status`
  );
}
