...
function supplierStoreUrl(id) {
  const pVisaId = localStorage.getItem("visaId");
  if (id) return `/visas/${pVisaId}/ledgers/${id}`;
  else return `/visas/${pVisaId}/ledgers`;
}
...
export function updateSingleStatus(supplier) {
  return http.get(
    `/visas/${auth.getCurrentUser().company_id}/ledgers/${supplier.id}/status`
  );
}
