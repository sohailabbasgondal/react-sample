...
function supplierStoreUrl(id) {
  if (id) return `/companies/${auth.getCurrentUser().company_id}/visas/${id}`;
  else return `/companies/${auth.getCurrentUser().company_id}/visas`;
}
...
export function updateSingleStatus(supplier) {
  return http.get(
    `/companies/${auth.getCurrentUser().company_id}/visas/${supplier.id}/status`
  );
}
