...
function supplierStoreUrl(id) {
  if (id) return `/companies/${auth.getCurrentUser().company_id}/rooms/${id}`;
  else return `/companies/${auth.getCurrentUser().company_id}/rooms`;
}
...
export function updateSingleStatus(supplier) {
  return http.get(
    `/companies/${auth.getCurrentUser().company_id}/rooms/${supplier.id}/status`
  );
}
