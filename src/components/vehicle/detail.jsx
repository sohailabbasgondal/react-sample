...
import { getSingle } from "../../services/vehicleService";
import { getAll, deleteSingle } from "../../services/vehicleDocumentService";
import { getLedgerTypes } from "../../services/vehicleLedgerTypeService";
import {
  saveSingle,
  getAllLedgers,
  deleteLedger
} from "../../services/vehicleLedgerService";
import VehicleLedgerTable from "./ledgerTable";
...
class VehicleDetail extends Form {
  state = {
    visa: {},
    visaId: "",
    documents: [],
  ...
  async componentDidMount() {
    ...
    localStorage.setItem("vehicleId", visaId);
  ...
  handleUpdate = type => {
    return this.props.history.replace(
      "/vehicles-documents/" + type.id + "/edit"
    );
  };

  handleUpdateLedger = type => {
    return this.props.history.replace("/vehicles-ledgers/" + type.id + "/edit");
  };
  ...
  doSubmit = async () => {
    try {
      ...
      this.props.history.push("/vehicles-accounts/view/" + this.state.visaId);
    } catch (ex) {
  ...
  render() {
    ...
    const url = "/vehicles-accounts/" + this.state.visaId + "/document/new/";
    return (
      <BlockUi tag="div" blocking={this.state.blocking}>
        ...
        <TableTitle title="Vehicle account detail" icon="tag" />
          ...
          <Grid.Row>
            <Grid.Column>
              <Table definition>
                <Table.Body>
                  <Table.Row>
                    <Table.Cell width={4}>Plate Number</Table.Cell>
                    <Table.Cell>{visa.plat_number}</Table.Cell>
                  </Table.Row>
                  <Table.Row>
        ...
        <TableTitle title="Vehicle account ledgers" icon="tag" />
