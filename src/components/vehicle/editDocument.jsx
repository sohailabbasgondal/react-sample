...
import { getSingle, updateSingle } from "../../services/vehicleDocumentService";
...
class VehicleDocumentEditForm extends Form {
  state = {
    imageUrl: false,
  ...
  doSubmit = async () => {
    try {
      ...
      var event = new Date(this.state.expiryDate);
      var utcDate = new Date(
        Date.UTC(event.getFullYear(), event.getMonth(), event.getDate())
      );

      date = JSON.stringify(utcDate);
      date = date.slice(1, 11);
  ...
  render() {
    ...
    return (
      <BlockUi tag="div" blocking={this.state.blocking}>
        <TableTitle title="Update vehicle document" icon="tag" />
        <center>
          <form
