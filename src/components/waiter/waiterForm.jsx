...
class WaiterForm extends Form {
  ...
  doSubmit = async () => {
    try {
      ...
      await saveUser(this.state.data, "waiter");
      toast.success("Waiter has been added successfully.");
  ...
  render() {
    return (
      <BlockUi tag="div" blocking={this.state.blocking}>
        <TableTitle title="Add waiter" icon="tag" />
        <form onSubmit={this.handleSubmit} className="ui error form">
          <Table>
            <Table.Header>
              <Table.Row>
                <Table.HeaderCell colspan="2">Add waiter</Table.HeaderCell>
              </Table.Row>
            </Table.Header>
            <Table.Body>
              <Table.Row width="8">
