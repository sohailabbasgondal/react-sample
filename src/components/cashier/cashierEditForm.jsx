...
class CashierEditForm extends Form {
  state = {
    data: {
      name: "",
      email: "",
  ...
  doSubmit = async () => {
    try {
      ...
      this.setState({ blocking: false });
      toast.success("Cashier has been updated successfully.");
  ...
  render() {
    return (
      <BlockUi tag="div" blocking={this.state.blocking}>
        <TableTitle title="Update cashier" icon="tag" />
        <form onSubmit={this.handleSubmit} className="ui error form">
          <Table>
            <Table.Header>
              <Table.Row>
                <Table.HeaderCell colSpan="2">Update cashier</Table.HeaderCell>
              </Table.Row>
            </Table.Header>
            <Table.Body>
              <Table.Row width="8">
