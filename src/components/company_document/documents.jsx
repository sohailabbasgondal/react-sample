...
import { getAll, deleteSingle } from "../../services/companyDocumentService";
...
class CompanyDocuments extends Component {
  ...
  async componentDidMount() {
    ...
    this.checkTotalTypes(types.data);
    this.setState({ types: types.data, blocking: false });
  }
  ...
  handleUpdate = type => {
    return this.props.history.replace(
      "/company-documents/" + type.id + "/edit"
    );
  };
        ...
        >
          <div>
            <center>
              <img src={emtpyUrl} />
              <div>
                <p>Documents not added yet.</p>
                <Link to="/company-documents/new" className="ui primary button">
                  New Document
                </Link>
              </div>
            </center>
    ...
    return (
      <BlockUi tag="div" blocking={this.state.blocking}>
        <Confirm
          ...
          size="mini"
        />
        <TableTitle title="Company Documents" icon="tag" />
