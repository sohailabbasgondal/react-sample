...
import { getLedger, updateSingle } from "../../services/yardLedgerService";
...
class YardLedgerEditForm extends Form {
  state = {
    imageUrl: false,
  ...
  async populateLedger() {
    try {
      ...
      this.setState({
        ...
        ledgerType: ledger.yard_ledger_type.id,
        data: {
  ...
  doSubmit = async () => {
    try {
      ...
      this.props.history.push("/yards-accounts/view/" + visaId);
    } catch (ex) {
  ...
  render() {
    ...
    return (
      <BlockUi tag="div" blocking={this.state.blocking}>
        <TableTitle title="Update yard ledger" icon="tag" />
        <center>
          <form
              ...
              <Table.Body>
                ...
                <Table.Row>
                  <Table.Cell>
                    ...
                    <Link
                      to={`/yards-accounts/view/${this.props.match.params.id}`}
...
export default YardLedgerEditForm;
