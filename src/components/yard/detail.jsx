...
import { getSingle } from "../../services/yardService";
import { getLedgerTypes } from "../../services/yardLedgerTypeService";
import {
  saveSingle,
  getAllLedgers,
  deleteLedger
} from "../../services/yardLedgerService";
import YardLedgerTable from "./ledgerTable";
import { Table, Grid, Confirm, Button } from "semantic-ui-react";
...
class YardDetail extends Form {
  ...
  async getVisaDetail(yardId) {
    try {
      this.setState({ blocking: true });

      const { data: visa } = await getSingle(yardId);
  ...
  async componentDidMount() {
    const yardId = this.props.match.params.id;
    await this.getVisaDetail(yardId);

    localStorage.setItem("yardId", yardId);
  ...
  handleUpdateLedger = type => {
    return this.props.history.replace("/yards-ledgers/" + type.id + "/edit");
  };
  handleCancel = () => {
  ...
  doSubmit = async () => {
    try {
      ...
      data.yard_ledger_type_id = this.state.ledgerType;
      ...
      this.props.history.push(
        "/yards-accounts/view/" + localStorage.getItem("yardId")
      );
    } catch (ex) {
  ...
  render() {
    ...
    return (
      <BlockUi tag="div" blocking={this.state.blocking}>
        ...
        <TableTitle title="Yard account detail" icon="tag" />
        ...
        <TableTitle title="Yard account ledgers" icon="tag" />
